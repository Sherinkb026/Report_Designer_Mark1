...
namespace Report_Mark1
{
    public partial class MainWindow : Window
    {
        ...
        private void SelectReport_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (currentData != null)
            {
                // Create the report template (using the ReportTemplate.xaml)
                ReportTemplate reportTemplate = new ReportTemplate();

                // Bind the report data to the template
                reportTemplate.LoadReportData(currentData.AsEnumerable());

                // Add the report template to the design surface
                Canvas.SetLeft(reportTemplate, 10);
                Canvas.SetTop(reportTemplate, 10);
                designSurface.Children.Add(reportTemplate);

                MessageBox.Show("Report template loaded into canvas.", "Success", MessageBoxButton.OK, MessageBoxImage.Information);
            }
        }
        ...
        private void SelectElement(UIElement element)
        {
            ...
            // Update UI selection (e.g., outline the selected element)
            foreach (var child in designSurface.Children)
            {
                if (child is Border b)
                    b.BorderBrush = System.Windows.Media.Brushes.Black;
        ...
        #region Button Event Handlers (Required)

        // Add the button handlers here:
